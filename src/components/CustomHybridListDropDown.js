...
const CustomHybridDropDown = ({ validationsBorder, valueHandle, name, data, label, inputValue, handleDropDown, dropDownVisible, closeDropDown }) => {
    const fonts=useFontStyles()

    const renderList = (item) => {
        console.log("itsauiaus=-=-=-=>",item.item)
        return (
            <TouchableOpacity onPress={() => valueHandle(item.item.productName)} style={RnStyles.labelContainer}>
                <Text style={[RnStyles.labeTextContainer,{fontFamily:fonts.SemiBold}]}>{item.item.productName}</Text>
            </TouchableOpacity>
    ...
    return (
        <View>
            ...
            <TouchableOpacity onPress={handleDropDown} style={[RnStyles.textInputContainer, { borderColor: validationsBorder ? "#ED3237" : "#D6D6D6" }]}>
                <Text style={[RnStyles.selectCropTextInput,{fontFamily:fonts.SemiBold}]}>{inputValue}</Text>
                ...
                <TouchableWithoutFeedback onPress={closeDropDown}>
                    <View style={RnStyles.modalMainContainer}>
                        <View style={RnStyles.modalSubContainer}>
                            <FlatList ListEmptyComponent={() => <Text style={{ color: "#000", fontSize: 15,  alignSelf: "center", marginBottom: 20,fontFamily:fonts.SemiBold }}>{translate("No_data_available")}</Text>} data={data} renderItem={renderList} />
                        </View>
                    </View>
...
export default CustomHybridDropDown
