...
const HomeScreenEmp = ({ route }) => {
  ...
  const farmerServiceHandleClose = () => {
    setFarmerServiceModalVisible(false)
  }
  ...
  const getMinDateEmployee = async () => {
    if (isConnected) {
      try {
        const headers = await GetApiHeaders();
        headers.authType = "JSONREQUEST";
        const url = APIConfig.BASE_URL + APIConfig.USERSGETMINDATEFROMUSERINFO;
  ...
  useFocusEffect(
    useCallback(() => {
      if (route?.params?.openFarmerServices) {
        //   // setFarmerServiceModalVisible(true);
    ...
    }, [route?.params?.title, route?.params?.response, route?.params?.openFarmerServices])
  );
  ...
  const renderServiceItems = ({ item }) => {
    return (
      <TouchableOpacity onPress={() => handleServiceClickNavigations(item.title, item.subTitle)} style={styles.serviceListContainer}>
        <View style={styles.serviceListSubContainer}>
          {item?.icon &&
            <Image source={{ uri: item?.icon }} style={styles.serviceImgs} />
          }
        </View>
        <View style={styles.serviceTextContainer}>
          {item?.subTitleTranslate &&
            <Text style={[styles.serviceSubText, { fontFamily: fonts.Regular, color: "#3A3A3A" }]}>{item?.subTitleTranslate ?? ""}</Text>
          }
          {item?.titleTranslate &&
            <Text style={[styles.serviceMainText, { color: item.subTitleColor, fontFamily: fonts.SemiBold }]}>{item?.titleTranslate ?? ""}</Text>
          }
        </View>
      </TouchableOpacity>
    );
  ...
  const renderFarmerService = (item) => {
    return (
      <Pressable>
        ...
        <Text style={[styles.serviceText, { marginLeft: 10, marginVertical: 10, fontFamily: fonts.SemiBold }]}>{item?.item?.sectionItemsTitleTranslated ?? ""}</Text>
        <FlatList
          // ListEmptyComponent={() => <Text style={styles.noDataText}>{translate('No_data_available')}</Text>}
          numColumns={4}
          data={item?.item?.sectionItemsList?.filter((item) => item.visible)}
          renderItem={renderServiceItems}
          keyExtractor={item => item.id.toString()}
          initialNumToRender={4}
        />
  ...
  return (
    <View style={styles.homeMainContainer}>
      ...
      <Modal animationType="slide" transparent visible={farmerServiceModalVisible}>
        <TouchableWithoutFeedback>
          <View style={styles.modalMainContainer}>
            <View style={styles.modalSubContainerMenuList}>
      ...
      <DateRangePickerModal
        ...
        minDateFromBE={minDate}
        maxDateFromBE={formattedDate}
